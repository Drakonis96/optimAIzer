  ...
  const renderMarkdown = (content: string, sourceId: string, className = 'markdown-body') => {
    ...
    const components = {
      a: ({ href, children, ...props }: any) => {
        if (href && /^\/api\/agents\/[^/]+\/documents\/[^/]+$/.test(href)) {
          return (
            <a
              href={href}
              download
              title={typeof children === 'string' ? children : 'Descargar'}
              className="inline-flex items-center justify-center w-7 h-7 rounded-md bg-zinc-200 hover:bg-zinc-300 dark:bg-zinc-700 dark:hover:bg-zinc-600 text-zinc-600 dark:text-zinc-300 transition-colors no-underline shadow-sm mt-1"
              {...props}
            >
